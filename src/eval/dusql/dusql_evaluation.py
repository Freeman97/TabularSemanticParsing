...
# from smbop.eval_final.utils import evaluate_NL2SQL
# from smbop.eval_final.utils import is_float
# from smbop.eval_final.utils import evaluate_NL2SQL_single

from src.eval.dusql.utils import evaluate_NL2SQL
from src.eval.dusql.utils import is_float
from src.eval.dusql.utils import evaluate_NL2SQL_single
...
def evaluate_complex_single(pred_str, gold_str, table_file, db_id):
    ...
    if complex_table_dict is None:
        complex_table_dict = {}
        with open(table_file, encoding='utf8') as ifs:
            table_list = json.load(ifs)
            for table in table_list:
                if table['db_id'] in complex_table_dict:
                    continue
                complex_table_dict[table['db_id']] = table
    evaluator = Evaluator()
    schema = Schema(complex_table_dict[db_id])
    gold_str = gold_str.replace('==', '=')
    gold_sql = get_sql(schema, gold_str, single_equal=True)
    ...
    try:
        ...
        pred_str = pred_str.replace('`', '')
        # pred_str = pred_str.replace('"', '')
        pred_sql = get_sql(schema, pred_str, single_equal=True)
    ...
    if 'cspider' in table_file:
        return evaluator.eval_exact_match(pred_sql, gold_sql, value_match=False)
    return evaluator.eval_exact_match(pred_sql, gold_sql, value_match=True)
...
def evaluate_complex(table, gold, predict, mode='exact', single_equal=False):
    ...
    with open(table, encoding='utf8') as ifs:
        table_list = json.load(ifs)
        table_dict = {}
        for table in table_list:
