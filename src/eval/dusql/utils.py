...
def evaluate_NL2SQL_single(pred_str, gold_str, table_dict, db_id):
    scores = {}
    scores_novalue = {}
    db = None
    db = table_dict[db_id]
    ...
    cols = [x[1] for x in db['column_names']]
    ...
    try:
        sql_gold = gold_str.replace('==', '=')
        sql_pred = pred_str.replace('==', '=')
        components_gold, sels_gold = query2sql(sql_gold, cols, single_equal=True)
        components_pred, sels_pred = query2sql(sql_pred, cols, single_equal=True)
