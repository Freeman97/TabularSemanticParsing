...
def generate_query_toks(data_path, use_back_quote=False):
    if 'csgsql' in data_path or 'dusql' in data_path or 'debug' in data_path or 'nl2sql' in data_path or 'cspider' in data_path:
        changed_flag = False
        with open(data_path, 'r') as f:
            ...
            for unit in tqdm(new_data):
                ...
                if 'dusql' in data_path or 'debug' in data_path or 'cspider' in data_path:
                    query_toks = tokenize_dusql(unit['query'], use_back_quote=use_back_quote)
                elif 'nl2sql' in data_path:
                    query_toks = tokenize_dusql(unit['query'], replace_double_quote=False, use_back_quote=use_back_quote)
                else:
                    query_toks = tokenize_query(unit['query'])
                unit['query_toks'] = query_toks
                unit['query_toks_no_value'] = query_toks
                unit['query'] = ' '.join(query_toks)
        # 替换原有内容
        if changed_flag:
